...
def __generateGroundTruth(tgt_mapping:dict[str, str], header_names:list[str], output_csv:str) -> None:
    """
        Creates a `.csv` file containing the mapped values specified in `tgt_mapping` with column headers from `header_names`.
        Writes this file to `output_csv`.
    """
...
def initialGeneration() -> None:
    ...
    ##  mapping 1
    __processHealthcareData(__TRUTH_MAPPING_1, resource.getDataFile("healthcare.csv"), resource.getDataFile("source_1.csv"))
    __generateGroundTruth(__TRUTH_MAPPING_1, __ground_truth_headers, resource.getDataFile("ground_truth_1.csv"))

    ##  mapping 2
    __processHealthcareData(__TRUTH_MAPPING_2, resource.getDataFile("healthcare.csv"), resource.getDataFile("source_2.csv"), random_id=True)
    ##  handle `random_state` column too?
    __generateGroundTruth(__TRUTH_MAPPING_2, __ground_truth_headers, resource.getDataFile("ground_truth_2.csv"))

    ##  mapping 3
    __processHealthcareData(__TRUTH_MAPPING_3, resource.getDataFile("healthcare.csv"), resource.getDataFile("source_3.csv"))
    __generateGroundTruth(__TRUTH_MAPPING_3, __ground_truth_headers, resource.getDataFile("ground_truth_3.csv"))
